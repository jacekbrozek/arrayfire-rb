...
static std::string classify(af::array arr, int k) {
    std::stringstream ss;
    if (expand_labels) {
...
static void setup_mnist(int *num_classes, int *num_train, int *num_test,
                        ...
                        float frac, std::string path) {
    std::vector<dim_t> idims;
    std::vector<float> idata;
    read_idx(idims, idata, (path + "images-subset").c_str());

    std::vector<dim_t> ldims;
    std::vector<unsigned> ldata;
    read_idx(ldims, ldata, (path + "labels-subset").c_str());
